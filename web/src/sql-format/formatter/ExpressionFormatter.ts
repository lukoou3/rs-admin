...
interface FormatNodeParams {
  isFunc?: boolean;
}

function defaultFormatNodeParams(): FormatNodeParams {
  return {isFunc: false}
}
...
export default class ExpressionFormatter {
  ...
  public format(nodes: AstNode[], parames: FormatNodeParams = defaultFormatNodeParams()): Layout {
    this.nodes = nodes;

    var preNode;
    for (this.index = 0; this.index < this.nodes.length; this.index++) {
      if ((preNode && preNode.type === NodeType.keyword && preNode.text === 'IF') && !parames.isFunc) {
        parames.isFunc = true;
        this.formatNode(this.nodes[this.index], parames);
        parames.isFunc = false;
      } else {
        this.formatNode(this.nodes[this.index], parames);
      }
      preNode = this.nodes[this.index];
    }
    return this.layout;
  }

  private formatNode(node: AstNode, parames: FormatNodeParams = defaultFormatNodeParams()) {
    this.formatComments(node.leadingComments);
    this.formatNodeWithoutComments(node, parames);
    this.formatComments(node.trailingComments);
  }

  private formatNodeWithoutComments(
    node: AstNode,
    parames: FormatNodeParams = defaultFormatNodeParams()
  ) {
    switch (node.type) {
      ...
      case NodeType.parenthesis:
        return this.formatParenthesis(node, parames);
      ...
      case NodeType.keyword:
        return this.formatKeywordNode(node, parames);
    }
  }
  ...
  private formatFunctionCall(node: FunctionCallNode) {
    ...
    this.formatNode(node.parenthesis, { isFunc: true });
  }
  ...
  private formatParenthesis(
    ...
    const inlineLayout = this.formatInlineExpression(node.children, parames);

    if (inlineLayout) {
      this.layout.add(node.openParen);
      this.layout.add(...inlineLayout.getLayoutItems());
      this.layout.add(WS.NO_SPACE, node.closeParen, WS.SPACE);
    } else if (parames.isFunc) {
      this.layout.add(node.openParen);
      this.layout = this.formatSubExpression(node.children, parames);
      this.layout.add(WS.NO_SPACE, node.closeParen, WS.SPACE);
    } else {
  ...
  private formatClauseInIndentedStyle(node: ClauseNode) {
    if (node.nameKw.text === 'FROM') {
      ...
      this.layout.add(WS.SPACE);
      this.layout = this.formatSubExpression(node.children);
    } else if (node.nameKw.text === 'WHERE' || node.nameKw.text === 'GROUP BY') {
      this.layout.add(WS.NEWLINE, WS.INDENT, this.showKw(node.nameKw), WS.SPACE);
      this.layout.indentation.increaseTopLevel();
      const oldInline = this.inline;
      this.inline = true;
      this.layout = this.formatSubExpression(node.children);
      this.inline = oldInline;
      this.layout.indentation.decreaseTopLevel();
    } else if (node.nameKw.text === 'WITH') {
      this.layout.add(WS.NEWLINE, WS.INDENT, this.showKw(node.nameKw), WS.SPACE);
      this.layout = this.formatSubExpression(node.children);
    } else {
  ...
  private formatSubExpression(nodes: AstNode[], parames: FormatNodeParams = defaultFormatNodeParams()): Layout {
    return new ExpressionFormatter({
      ...
      layout: this.layout,
      inline: this.inline,
    }).format(nodes, parames);
  }

  private formatInlineExpression(nodes: AstNode[], parames: FormatNodeParams = defaultFormatNodeParams()): Layout | undefined {
  ...
  private formatKeywordNode(
    ...
    switch (node.tokenType) {
      ...
      case TokenType.OR:
      case TokenType.XOR:
        return this.formatLogicalOperator(node, parames);
      default:
  ...
  private formatKeyword(node: KeywordNode) {
    if (node.text === 'IF') {
      this.layout.add(this.showKw(node));
    } else {
  ...
  private formatLogicalOperator(
    ...
    if (parames.isFunc) {
      // and 不换行
      this.layout.add(this.showKw(node), WS.SPACE);
    } else if (this.cfg.logicalOperatorNewline === 'before') {
      if (isTabularStyle(this.cfg)) {
    ...
    } else if (this.cfg.logicalOperatorNewline === 'inline') {
      // and 不换行
      this.layout.add(this.showKw(node), WS.SPACE);
    } else {
