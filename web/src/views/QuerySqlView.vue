...
async function save() {
  ...
  try {
    if (mode.value === "create") {
      await apiFetch("/api/sql-queries", {
        method: "POST",
        body: JSON.stringify(body),
      });
      ElMessage.success("已创建");
    } else if (mode.value === "edit" && editingId.value != null) {
      await apiFetch(`/api/sql-queries/${editingId.value}`, {
        method: "PUT",
        body: JSON.stringify(body),
      });
...
async function removeRow(row: Row) {
  try {
    ...
    await apiFetch(`/api/sql-queries/${row.id}`, { method: "DELETE" });
...
async function loadCateDict() {
  try {
    cateOptions.value = await getDictionary("querysql_cate");
  } catch (e) {
    ElMessage.error(errMsg(e));
  }
}
  ...
  >
    ...
    >
      ...
      <el-form-item label="描述:">
        <el-input v-model="form.desc" type="textarea" :rows="4" :readonly="isView" />
      </el-form-item>
    </el-form>
    <template #footer>
